...
const convertUTCToIST = (utcDateTime) => {
  ...
  useEffect(()=>{
    axios.get('http://13.127.182.122:5004/bills/getAllBills').then(res=>{
        setRecodes(res.data.data)
    })
//    axios.get(`http://13.127.182.122:5004/bills/Search?query=${modalData.partyName}`).then((res)=>{
...
//    })
  ...
  const submitData = () => {
    ...
    // Update modalData with selected start and end dates
    const dataToSend = {
        startDate: selectedStartDate ? selectedStartDate.toISOString() : "", // Convert date to ISO string format
        endDate: selectedEndDate ? selectedEndDate.toISOString() : "",
        partyName:modalData.partyName
    };
console.log("333",dataToSend)
setModalData({
})
    // // Make sure selectedStartDate and selectedEndDate are not null before submitting
    if (selectedStartDate && selectedEndDate) {
        axios.post('http://13.127.182.122:5004/bills/getReportsBills', dataToSend)
            .then((response) => {
